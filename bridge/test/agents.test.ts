...
import { validateNormalize, emptyRecord, buildQuerySeed } from "../src/agents.ts";
import { schemaFor, SCHEMAS, toAnswerJsonSchema, DOC_TYPES } from "../src/schemas.ts";
...
test("emptyRecord seeds sensible defaults", () => {
  const rec = emptyRecord("id1", "f.pdf", "application/pdf");
  assert.equal(rec.id, "id1");
  assert.equal(rec.docType, "generic");
  assert.deepEqual(rec.fields, []);
  assert.ok(rec.meta.processedAt);
});
